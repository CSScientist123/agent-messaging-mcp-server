...
**The adapters** raise their own, all of which mean "the remote could not do this, and I will not pretend otherwise": `antigravity_session_unreachable`, `antigravity_project_unknown`, `antigravity_project_unreadable`, `antigravity_session_stuck_in_editor`, `approval_is_an_error`, `permissions_view_did_not_open` (Antigravity); `no_remote_cancel` (Claude Science); `no_adapter_for_code` and `unknown_source_prefix` (the registry).
